...
public:
  ...
  dberr_t insert(uint32_t page_no, mtr_t *mtr)
  {
    ...
    ulint m_size= m_old_xdes_pages.size();
    if (m_size > m_index && m_old_xdes_pages[m_index] != nullptr)
      return DB_SUCCESS;
                        ...
                        uint32_t threshold, mtr_t *mtr)
{
  ...
  for (uint32_t i= len; i > 0; i--)
  {
    ...
    if (addr.page < threshold)
    {
	  ...
	  header, hdr_offset, addr, last_valid_addr,
	  skip_len, mtr);
	if (err) return err;
        skip_len= 0;
      }
...
__attribute__((warn_unused_result))
static
dberr_t fsp_xdes_reset(fil_space_t *space, ulint threshold, mtr_t *mtr)
{
  dberr_t err= DB_SUCCESS;
  buf_block_t *block= nullptr;
  ...
  for (uint32_t cur_extent=
       ...
       cur_extent >= threshold; cur_extent-= FSP_EXTENT_SIZE)
  {
    uint32_t cur_descr_page= xdes_calc_descriptor_page(0, cur_extent);
    /* Ignore the truncated extent descriptor page */
    if (cur_descr_page >= threshold) continue;

    ulint descr_offset= XDES_ARR_OFFSET + XDES_SIZE
      * xdes_calc_descriptor_index(0, cur_extent);
    if (!block || cur_descr_page != block->page.id().page_no())
    {
      block= fsp_get_latched_xdes_page(cur_descr_page, mtr, &err);
      if (!block)
        return err;
    }
    mtr->memset(block, descr_offset, XDES_SIZE, 0);
  }
  return err;
}
