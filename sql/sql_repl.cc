...
struct binlog_send_info {
  ...
  binlog_send_info(THD *thd_arg, String *packet_arg, ushort flags_arg,
    ...
    : thd(thd_arg), net(&thd_arg->net), packet(packet_arg),
      ...
      should_stop(false), is_until_exclusive(false)
  {
    error_text[0] = 0;
...
static bool
get_slave_gtid_until_exclusive(THD *thd)
{
  bool null_value;

  const LEX_CSTRING name= { STRING_WITH_LEN("slave_gtid_until_exclusive") };
  user_var_entry *entry=
    (user_var_entry*) my_hash_search(&thd->user_vars, (uchar*) name.str,
                                     name.length);
  return entry && entry->val_int(&null_value) && !null_value;
          ...
          */
              ...
              info->is_until_exclusive)
          {
            /*
              ...
              Or the until condition is exclusive.
            */
            info->gtid_skip_group = (flags2 & Gtid_log_event::FL_STANDALONE ?
                                GTID_SKIP_STANDALONE : GTID_SKIP_TRANSACTION);
          }
                              ...
                              my_off_t *pos)
{
  ...
  if (info->using_gtid_state)
  {
    ...
    if (get_slave_until_gtid(thd, &slave_until_gtid_str))
    {
      ...
      info->is_until_exclusive= get_slave_gtid_until_exclusive(thd);
    }
  }
      ...
      */
      if (thread_mask & SLAVE_SQL)
      {
        ...
        else if (thd->lex->mi.gtid_pos_str.str)
        {
          ...
          mi->rli.is_until_exclusive= thd->lex->mi.is_until_exclusive;
        }
        else
