  ...
  if (mi->using_gtid != Master_info::USE_GTID_NO)
  {
    ...
    */
    ...
    if (mi->rli.until_condition == Relay_log_info::UNTIL_GTID)
    {
      ...
      */
      query_str.length(0);
      if (query_str.append(STRING_WITH_LEN("SET @slave_gtid_until_exclusive="),
                           system_charset_info) ||
          query_str.append_ulonglong(mi->rli.is_until_exclusive))
      {
        err_code= ER_OUTOFMEMORY;
        errmsg= "The slave I/O thread stops because a fatal out-of-memory error "
          "is encountered when it tries to set @slave_gtid_until_exclusive.";
        sprintf(err_buff, "%s Error: Out of memory", errmsg);
        goto err;
      }
      ...
      if (unlikely(rc))
      {
        ...
        {
          mi->report(ERROR_LEVEL, err_code, NULL,
                     "Setting @slave_gtid_until_exclusive failed with "
                     "error: %s", mysql_error(mysql));
          goto network_err;
        }
        else
        {
          /* Fatal error */
          errmsg= "The slave I/O thread stops because a fatal error is "
            "encountered when it tries to set @slave_gtid_until_exclusive.";
          sprintf(err_buff, "%s Error: %s", errmsg, mysql_error(mysql));
          goto err;
        }
      }
